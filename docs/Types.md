...
| Float | `3.14`, `1.0e21` | 64-bit finite values, scientific notation supported |
...
Ordinal and ISO week dates aren't accepted. A leap second (`60`) is preserved in text and converts
to the first second of the following minute. A time-only value cannot include a zone.
...
Int stores 64 bits with no separate unsigned type. Literals above `2^63-1` keep their unsigned bit
pattern and read back exactly through `GetValue<uint64_t>()`. They re-serialize in signed form:
`18446744073709551615` is written back as `-1`, with the same bits.
...
returns a mutable span over those structs. Persist the named fields because the bitfield object
representation is compiler-specific.
...
widen to floats. A mix with no common type, such as `1|true` or `1|"a"`, is an error.
...
Pack components share one `Entry`, so they don't have comments, nesting or separate addresses.
Packs work well for coordinates and color channels. Use an array for separate values.

## Strings

- Either `"double"` or `'single'` quotes
- Escapes like `\t`, `\n`, `\"`, `\'`, `\\` and so on (`\u`/`\U` remain literal)
